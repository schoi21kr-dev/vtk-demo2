...
구성 (데모 1과 기기 역할 교체): PC = 키패드 B(숫자표시) + QR / 휴대폰 = 키패드 A(무표시 좌표입력) + 위치 강조 토글
...
- 비밀번호 위치 강조 토글은 휴대폰(키패드 A) 화면에 있으며 기본값은 "꺼짐"임. 강조를 켜면 무표시 패드에 정답 위치가 표시되므로 시연 취지상 발표자 설명용으로만 잠깐 켜고, 실전 입력 시에는 꺼둘 것. (PC 화면에는 강조 기능이 없고 숫자판만 표시됨.)
- QR·세션은 자동 갱신되지 않으며, 필요 시 PC의 "새 챌린지 (새 π 생성)" 버튼으로 수동 갱신함.
